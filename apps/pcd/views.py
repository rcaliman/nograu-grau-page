...
def pcd_calculos_do_dia(request):
    ...
    for dado in dados:
        ...
        taxa_juros = (
            calcula_taxa_de_juros(
                dado.quantidade_parcelas, valor_parcela, valor_emprestado
            ),
        )
        meses_em_ser = calcula_meses_em_ser(proxima_parcela, ultima_parcela)
        saldo_devedor = calcula_saldo_devedor(
            valor_parcela, taxa_juros[0], meses_em_ser
        )
        resultados.append(
            {
    ...
    return render(
        ...
        "pcd/calculos_do_dia.html",
        {
            ...
            "subtitle": [
                "Dados dos cálculos efetuados hoje",
            ],
            "quantidade_calculos": quantidade_calculos,
        },
    )
...
def pcd_result(request):
    ...
    if len(POST) <= 2:
        ...
        if form.is_valid():
            ...
            today = datetime.now().strftime("%Y-%m-%d")
            quantidade_calculos = (
                ModelPCD.objects.all().filter(data_calculo=today).count()
            )
        ...
        if form.is_valid():
            ...
            if (
                int(resultado_pcd["quantidade_de_parcelas"]) < 1
                or int(resultado_pcd["meses_em_ser"]) < 1
...
def calcula_pcd(POST):
    ...
    taxa_juros = calcula_taxa_de_juros(
        POST["quantidade_parcelas"], POST["valor_parcela"], POST["valor_emprestado"]
    )
...
def calcula_taxa_de_juros(
    quantidade_parcelas, valor_parcela, valor_emprestado
) -> float:
    quantidade_parcelas = int(quantidade_parcelas)
    valor_parcela = float(valor_parcela)
    valor_emprestado = float(valor_emprestado)
    taxa_de_juros = rate(quantidade_parcelas, -valor_parcela, valor_emprestado, 0) * 100
...
def busca_banco(codigo_banco):
    ...
    with open(arquivo, encoding="utf-8") as f:
        ...
        for _, linha in reader.iterrows():
            if int(codigo_banco) == int(linha.Número_Código):
                return {
